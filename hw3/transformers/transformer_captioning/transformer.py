...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        N, S, D = query.shape # D is embed_dim
        N, T, D = value.shape
        # attn_mask is a binary mask of shape (N, S, T)
        ...
        #Expected shape of dot_product is (N, S, T) N is batch size, S is sequence length, T is 
        dot_product = torch.bmm(query, key.transpose(1, 2)) / math.sqrt(d_k) # shape (N, S, T)
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # attn_mask is a binary mask of shape 
        # print("Attention mask in Multihead", attn_mask.shape)
        #project query, key and value
        ...
        output = y.transpose(1, 2).contiguous().view(N, S, D) # verify this
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        self.encoding = torch.zeros(max_len, embed_dim).to('cuda')
        idx_tensor = torch.arange(0, max_len).float().unsqueeze(1)
        denominator = torch.pow(1e4, torch.arange(0, embed_dim, 2).float() / embed_dim)
        
        # Compute the encoding
        self.encoding[:, 0::2] = torch.sin(idx_tensor / denominator)
        self.encoding[:, 1::2] = torch.cos(idx_tensor / denominator)
