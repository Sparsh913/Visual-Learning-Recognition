...
pandaUid = p.loadURDF("pybullet-playground-20240518T073454Z-001/pybullet-playground/urdf/robotiq_2f_85.urdf", [0, 0, 0],  p.getQuaternionFromEuler([0,0,0]), flags=flags, useFixedBase=False)
...
print("number of links:", p.getNumJoints(pandaUid))

# there are a total of 10 links in the gripper, save the point cloud of each link

# # hide all other links except link 9
for i in range(11):
    if i != 0:
        p.changeVisualShape(pandaUid, i, rgbaColor=[0, 0, 0, 0])
        
# # Hide the base too
p.changeVisualShape(pandaUid, -1, rgbaColor=[0, 0, 0, 0])
...
while True:
    ...
    o3d.io.write_point_cloud("gripper/link_0.ply", pcd)
