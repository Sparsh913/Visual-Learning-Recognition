...
class UpSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # print("upscale factor", self.upscale_factor)
        x = x.repeat(1, int(self.upscale_factor**2), 1, 1) # (batch, channel*upscale_factor^2, height, width)
        # print("x.shape: ", x.shape)
        # Use torch.nn.PixelShuffle to form an output of dimension
        # (batch, channel, height*upscale_factor, width*upscale_factor)
...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        # pass
        x = F.pixel_unshuffle(x, self.downscale_ratio) # (batch, channel*downscale_factor^2, height, width)
        x = x.view(int(self.downscale_ratio**2), x.size(0), x.size(1)//(int(self.downscale_ratio**2)), x.size(2), x.size(3))
        x = x.mean(dim=0) # (batch, channel, height, width)
        # print("x.shape in DownSampleConv2D: ", x.shape) 
        return self.conv(x)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            UpSampleConv2D(input_channels=n_filters, kernel_size=3, n_filters=n_filters, upscale_factor=2, padding=1)
        )
        self.upsample_residual = UpSampleConv2D(input_channels=input_channels, kernel_size=1, n_filters=n_filters, upscale_factor=2, padding=0)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers_d = nn.Sequential(
            ...
            nn.ReLU(),
            DownSampleConv2D(n_filters, kernel_size=(3,3), n_filters=n_filters, padding=(1,1)))
...
class ResBlock(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        # pass
        x += self.layers(x) ## Verify if this residual connection is correct
        return x
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def __init__(self, starting_image_size=4, n_samples = 256):
        ...
        self.layers = nn.Sequential(
            ResBlockUp(input_channels=int(128), kernel_size=int(3), n_filters=int(128)), 
    ...
    @torch.jit.script_method
    def forward(self, n_samples: int = 256): #changed n_samples from 1024
        ##################################################################
        ...
        latents = torch.randn(n_samples, 128).cuda()
        return self.forward_given_samples(latents)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        # pass
        # print("x.shape in Discriminator forward: ", x.shape) # (batch, channel, height, width)  
        x = self.layers(x)
        x = x.sum(dim=(2, 3))
        x = self.dense(x)
        return x
