...
class Encoder(nn.Module):
    ...
    def __init__(self, input_shape, latent_dim):
        ...
        self.convs = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)), # (batch, 32, input_shape[1], input_shape[2])
            nn.ReLU(),
            nn.Conv2d(32, 64, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)), # (batch, 64, input_shape[1]//2, input_shape[2]//2)
            nn.ReLU(),
            nn.Conv2d(64, 128, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)), # (batch, 128, input_shape[1]//4, input_shape[2]//4)
            nn.ReLU(),
            nn.Conv2d(128, 256, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)) # (batch, 256, input_shape[1]//8, input_shape[2]//8)
        )
...
class VAEEncoder(Encoder):
    def __init__(self, input_shape, latent_dim):
        ...
        ##################################################################
        # self.fc = None
        # self.fc = nn.Linear(input_shape[0]*input_shape[1]*input_shape[2], 2*latent_dim)
    ...
    def forward(self, x):
        ...
        mu1 = self.fc(mu)
        mu = mu1[:, :self.latent_dim]
        log_std = mu1[:, self.latent_dim:]
        log_std = torch.clamp(log_std, -4, 15) # clamp log_std to be in the range [-4, 15] to avoid numerical issues
        # return mu, log_std
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim, output_shape):
        ...
        ##################################################################
        self.base_size = 0 # base size of input to fc layer
        # self.deconvs = None
        # self.fc = None
        
        # self.base_size = self.latent_dim * 
        self.fc = nn.Linear(self.latent_dim, 256*4*4)
        self.deconvs = nn.Sequential(
            nn.ReLU(),
            nn.ConvTranspose2d(256, 128, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1)), # from (batch, 256, 4, 4) to (batch, 128, 8, 8)
            nn.ReLU(),
            nn.ConvTranspose2d(128, 64, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1)), # from (batch, 128, 8, 8) to (batch, 64, 16, 16)
            nn.ReLU(),
            nn.ConvTranspose2d(64, 32, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1)), # from (batch, 64, 16, 16) to (batch, 32, 32, 32)
            nn.ReLU(),
            nn.Conv2d(32, 3, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)) # from (batch, 32, 32, 32) to (batch, 3, 32, 32)
        )
