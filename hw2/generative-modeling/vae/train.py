...
def vae_loss(model, x, beta = 1):
    ...
    # kl_loss = 0.5 * torch.sum(mu**2 + std**2 - 1 - torch.log(std**2))
    # kl_loss = kl_loss.mean(dim=0)
    kl_loss = -0.5 * torch.sum(1 + 2*log_std - mu**2 - torch.exp(2*log_std), dim=1).mean()
...
def linear_beta_scheduler(max_epochs=None, target_val = 1):
    ...
    def _helper(epoch):
        # pass
        # Another way to improve the quality of samples is to use an annealing scheme for beta.
        # One simple scheme is to start with a small value of beta and increase it linearly over the course of training.
